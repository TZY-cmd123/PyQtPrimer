...
class Demo(QWidget):  # 1该类继承QWidget，可以将QWidget看作是一种毛坯房，还没有装修，而我们往其中放入QPushButton、QLabel等控件就相当于在装修这间毛坯房。类似的毛坯房还有QMainWindow和QDialog，之后章节再讲述；
    ...
    def __init__(self):
        ...
        self.button.clicked.connect(self.change_text)           # 3clicked(按钮被点击)是该控件的一个信号，所以通用的公式可以是：widget.signal.connect(slot)；
        self.button.clicked.connect(self.Mytest3)               # 同一个信号可以连接多个信号

        self.my_signal.connect(self.Mytest4)                # 发出自定义信号

    def mousePressEvent(self, QMouseEvent):  # 监视鼠标，如果按下就释放信号
        self.my_signal.emit()
