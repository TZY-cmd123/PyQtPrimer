...
class SliderDemo(QWidget):
    def __init__(self):
        ...
        self.slider_1 = QSlider(Qt.Horizontal, self)  # 1水平的滑动条
        self.slider_1.setRange(0, 100)  # 2范围
        self.slider_1.valueChanged.connect(lambda: self.on_change_func(self.slider_1))  # 3滑动时的监听器
        ...
        self.label = QLabel('0', self)  # 6用来标记数字
    ...
    def on_change_func(self, slider):  # 7当其中一个变化时，使得另一个变化
...
class DialDemo(QWidget):
    def __init__(self):
        super(DialDemo, self).__init__()
        self.setWindowTitle('QDial')                            # 1设置标题
        self.dial = QDial(self)
        self.dial.setFixedSize(100, 100)                        # 2设置表盘的位置
        self.dial.setRange(0, 100)                              # 3设置范围
        self.dial.setNotchesVisible(True)                       # 4显示刻度
        self.dial.valueChanged.connect(self.on_change_func)     # 5监听器
